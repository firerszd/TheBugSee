    ...
    public class CCDebugWindow : EditorWindow
    {
        ...
        internal void OnGUI()
        {
            ...
            foreach (var key in btnDic.Keys)
            {
                ...
                bool click = GUILayout.Button(content, getCurStyleByTypeStr("button", 15, curColor),
                    GUILayout.Width(100));
                GUILayout.Space(2);
                ...
                for (int i = 0; i < item.param.Length; i++)
                {
                    ...
                    else if (types == typeof(Int32))
                    {
                        ...
                        else
                        {
                            // 滑动框
                            vint = ToInt(
                                GUILayout.HorizontalSlider(ToInt(item.param[i]), min, max, GUILayout.Width(70)));
                        }
                        ...
                        GUILayout.Label($"{ToFloat(item.param[i]):F}", getCurStyleByTypeStr("label", 15, Color.white),
                            GUILayout.Width(50));
                        numberCount++;
                    }
                    ...
                    {
                        var vstr = GUILayout.TextArea(ToStr(item.param[i]),
                            getCurStyleByTypeStr("textarea", 15, curColor), GUILayout.Width(120));
                ...
                if (!string.IsNullOrEmpty(item.btnAttribute.Tip))
                {
                    ...
                    show = EditorGUILayout.Foldout(show, "提示",
                        getCurStyleByTypeStr("foldout", 15, Color.white)); // 定义折叠菜单
                    if (show)
                    {
        ...
        public GUIStyle getCurStyleByTypeStr(string type, int size = 20, Color? color = null,
            TextAnchor style = TextAnchor.MiddleLeft)
        {
